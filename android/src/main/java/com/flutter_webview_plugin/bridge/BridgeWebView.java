...
//    }
     ...
     * @param callBack
     */
    private void callHandler(String handlerName, String data, CallBackFunction callBack) {
    ...
    public void registerHandler(final String handlerName, final JsHandler handler) {
        registerHandler(handlerName, new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                if (handler != null) {
                    handler.onHandle(handlerName, data, function);
                }
            }
        });
    }
     ...
     * @param handler      回调接口
     */
    public void registerHandlers(final List<String> handlerNames, final JsHandler handler) {
        if (handler != null) {
    ...
    public void callHandler(final String handlerName, String javaData, final JavaCallHandler handler) {
        callHandler(handlerName, javaData, new CallBackFunction() {
            @Override
            public void onCallBack(String data) {
                if (handler != null) {
                    handler.onHandle(handlerName, data);
                }
            }
        });
    }
    ...
    public void callHandler(final Map<String, String> handlerInfos, final JavaCallHandler handler) {
        if (handler != null) {
            for (final Map.Entry<String, String> entry : handlerInfos.entrySet()) {
                callHandler(entry.getKey(), entry.getValue(), new CallBackFunction() {
                    @Override
                    public void onCallBack(String data) {
                        handler.onHandle(entry.getKey(), data);
                    }
                });
            }
        }
